...
Strips invisible Unicode zero-width tokens (\\u200B, \\uFEFF, \\u2060, etc.), non-standard whitespace,
and homoglyphs from AI outputs generated by Anthropic Claude, ChatGPT, or external models.
...
function inspectContent(target) {
  ...
  // Check zero width
  const zw = content.match(/[\u200B\u200C\u200D\uFEFF\u2060]|\\u(?:200[bcd]|feff|2060)/g);
  if (zw) {
    threats.push(`Invisible zero-width watermarks detected (${zw.length} instance${zw.length > 1 ? 's' : ''})`);
  }
...
async function run() {
  ...
  // Check if user asked for help on a command like `npx aiscrubber scrub --help`
